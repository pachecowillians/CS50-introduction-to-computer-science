...
def history():
    ...
    operations = db.execute(
        "select symbol, shares, price, date from operations where id_user = :user order by date desc", user=session["user_id"])
...
def quote():
    ...
    if request.method == "GET":
        ...
        if not symbol:
            ...
            if not value:
                return apology("The symbol doesn't exists")
            else:
...
def changepassword():
    ...
    if request.method == "GET":
        ...
        # If the confirmation password is diferent than new password
            ...
            old = db.execute("select hash from users where id = :user", user=session["user_id"])[0]['hash']
            ...
            if not check_password_hash(old, oldPass):
                ...
                db.execute("update users set hash = :passwd where id = :user",
...
def addcash():
    ...
    if request.method == "GET":
        ...
        if not cash:
            return apology("The cash field is empty")
        # If the value is negative
        elif float(cash) <= 0:
            return apology("The cash value must be positive")
        else:
            # Update the cash value and add the cash
            db.execute("update users set cash = cash + :cash where id = :user", cash=float(cash), user=session["user_id"])
            # Convert cash to usd notation
            cash = usd(float(cash))
            # Show to user that the momney were added to account
            flash(f'{cash} added!')
            # Go back to home
            return redirect("/")
